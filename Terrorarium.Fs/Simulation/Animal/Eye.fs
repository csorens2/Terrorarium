...
module Eye = 
    let New config = {Eye.FOVRange = config.EyeFOVRange; FOVAngle = config.EyeFOVAngle; Cells = config.EyeCells}

    let ProcessVision (position:Point2D) (facing:Vector2D) food eye = 
        let foodInVision food = 
        ...
        let getCellForFood food = 
            let foodVector = position.VectorTo(food.Position)
            let foodAngle = XAxisVector.SignedAngleTo(foodVector, false, true)
            let shiftedFoodAngle = foodAngle - XAxisVector.SignedAngleTo(facing, false, true) + (eye.FOVAngle / 2.0)
            let rangePercentage = (shiftedFoodAngle.Radians / eye.FOVAngle.Radians)
        ...
        let cellEnergy cellFood = Array.fold (fun acc next -> acc + (1.0 - (position.DistanceTo(next.Position) / eye.FOVRange))) 0.0 cellFood
        ...
        let rec getWrappedFood remainingFood = seq {
            if not (List.isEmpty remainingFood) then
                ...
                yield! List.map(fun x -> {nextFood with Position = nextFood.Position + x}) translationVecs
        ...
        [|0..eye.Cells - 1|]
        |> Array.map (fun cell -> if eyeCellDict.ContainsKey(cell) then eyeCellDict[cell] else 0)