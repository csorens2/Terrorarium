...
module Animal = 
    let New config brain = {
        Animal.Position = Point2D(Random.Shared.NextDouble(), Random.Shared.NextDouble())
    ...
    let Random config = New config (Brain.Random config)

    let FromChromosome config chromosome = New config (Brain.FromChromosome config chromosome)

    let AsChromosome animal = Brain.AsChromosome animal.Brain
    ...
    let ProcessMovement animal = 
        ...
        let nextPoint = Point2D((ExtraFunctions.Wrap nextPos.X 0.0 1.0),(ExtraFunctions.Wrap nextPos.Y 0.0 1.0))
